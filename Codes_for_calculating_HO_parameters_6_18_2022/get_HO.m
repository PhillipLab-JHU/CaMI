...
n_tot=size(xy_N,1)/N; dt=5; %n_tot=Total number of cells; dt = actual time difference b/w two time points
clear i; 
...
cell_N(:,:)=[MSD10 MSD100 Pp Pnp Dp Dnp Dtot phi];
...
cell_N_M(:,:)=[zscore(log(MSD10)) zscore(log(MSD100)) zscore(log(Pp)) zscore(log(Pnp)) zscore(log(Dp)) zscore(log(Dnp)) zscore(log(Dtot)) zscore(log(phi))];
