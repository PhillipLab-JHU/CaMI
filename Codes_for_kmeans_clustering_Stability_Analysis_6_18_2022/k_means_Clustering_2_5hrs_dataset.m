...
[idx,C,sumdist,D] = kmeans(cell_M_2_5hrs,n_clust,'Distance','correlation','Display','final','MaxIter',1000);

% Plot out the Silhouette values for the n_clusters
figure(2)
[silh,h] = silhouette(cell_M_2_5hrs,idx,'correlation'); set(gca,'YTick',[]);
% Calculate average Silhouette value per cell. NOTE: sum(silh) sums over all the cells:
Sum_Silh=sum(silh)/max(size(cell_M_2_5hrs))
...
for bin=1:max(idx) %max(idx) is the number of bins
    ...
    %subplot(4,4,bin)
    d_MSD10=d_MSD10+sum((C_MSD10(bin,1)-cell_M_2_5hrs(temp_bin,1)).^2);
    d_MSD100=d_MSD100+sum((C_MSD100(bin,1)-cell_M_2_5hrs(temp_bin,2)).^2);
    d_Pp=d_Pp+sum((C_Pp(bin,1)-cell_M_2_5hrs(temp_bin,3)).^2);
    d_Pnp=d_Pnp+sum((C_Pnp(bin,1)-cell_M_2_5hrs(temp_bin,4)).^2);
    d_Dp=d_Dp+sum((C_Dp(bin,1)-cell_M_2_5hrs(temp_bin,5)).^2);
    d_Dnp=d_Dnp+sum((C_Dnp(bin,1)-cell_M_2_5hrs(temp_bin,6)).^2);
    d_Dtot=d_Dtot+sum((C_Dtot(bin,1)-cell_M_2_5hrs(temp_bin,7)).^2);
    d_phi=d_phi+sum((C_phi(bin,1)-cell_M_2_5hrs(temp_bin,8)).^2);   
end
...
% clusters
d=d./max(size(cell_M_2_5hrs))./n_clust;
